...
Socket::Socket(SOCKHANDLE id) : id(id) {}
...
int Socket::create(int port, addr_family family, sock_type type) {
  ...
  switch (family) {
  case ace::ACE_AF_INET:
    hints.ai_family = AF_INET;
    break;
  case ace::ACE_AF_INET6:
    hints.ai_family = AF_INET6;
    break;
  case ace::ACE_AF_UNSPEC:
  ...
  switch (type) {
  case ace::ACE_SOCK_STREAM:
    hints.ai_socktype = SOCK_STREAM;
    break;
  case ace::ACE_SOCK_DATAGRAM:
...
Socket Socket::accept() {
  ...
  if (sock == INVALID_SOCKET) {
    ...
    case WSAEINPROGRESS:
      ...
      break;
    case WSAECONNRESET:
      printf("An incoming connection was indicated, but was subsequently "
             "terminated by the remote peer prior to accepting the call.\n");
      break;
    case WSAEFAULT:
      printf("The addrlen parameter is too small or addr is not a valid part "
    ...
    case WSAEINVAL:
      ...
      break;
    case WSAEMFILE:
      printf("The queue is nonempty upon entry to accept and there are no "
    ...
    case WSAENOTSOCK:
      ...
      break;
    case WSAEOPNOTSUPP:
      printf("The referenced socket is not a type that supports "
             "connection-oriented service.\n");
      break;
    case WSAEWOULDBLOCK:
      printf("The socket is marked as nonblocking and no connections are "
...
int Socket::close() {
  ...
  if (status != 0) {
    ...
    case WSAENETDOWN:
      ...
      break;
    case WSAENOTSOCK:
      printf("This object is not describing a socket.\n");
      break;
    case WSAEINPROGRESS:
      ...
      break;
    case WSAEINTR:
      printf("The (blocking) Windows Socket 1.1 call was canceled through "
             "WSACancelBlockingCall.\n");
      break;
    case WSAEWOULDBLOCK:
      printf("The socket is marked as nonblocking, but the l_onoff member of "
             "the linger structure is set to nonzero and the l_linger member "
             "of the linger structure is set to a nonzero timeout value.\n");
      break;
    default:
      printf("Unknown error.\n");
      break;
...
int Socket::shutdown() const {
  ...
  if (status != 0) {
    ...
    case WSAECONNABORTED:
      printf("The virtual circuit was terminated due to a time-out or other "
             "failure. The application should close the socket as it is no "
             "longer usable. This error applies only to a connection - "
             "oriented socket.\n");
      break;
    case WSAECONNRESET:
      printf("The virtual circuit was reset by the remote side executing a "
             "hard or abortive close. The application should close the socket "
             "as it is no longer usable. This error applies only to a "
    ...
    case WSAEINTR:
      ...
      break;
    case WSAENOTCONN:
      printf("The socket is not connected. This error applies only to a "
             "connection-oriented socket.\n");
      break;
    case WSAENETDOWN:
      printf("The network subsystem has failed.\n");
...
bool Socket::valid() const { return this->id != INVALID_SOCKET; }
